...
if ($Acces) {
	if (isset($_POST['UID']) && isset($_POST['Nom']) && isset($_POST['Email'])) Edit();
...
function Edit() {
		...
		$Logs->add($_SESSION['Nom'], 'editAdmin', $LogToSend);

		include ('../include/connexiondb_site.php');
		$req = $bdd->prepare("UPDATE Utilisateurs SET UID=:UID, Nom=:Nom, Email=:Email, PERM_View_User=:PERM_View_User, PERM_Edit_User=:PERM_Edit_User, PERM_View_Vehicle=:PERM_View_Vehicle, PERM_Edit_Vehicle=:PERM_Edit_Vehicle, PERM_View_Gang=:PERM_View_Gang, PERM_Edit_Gang=:PERM_Edit_Gang, PERM_View_Home=:PERM_View_Home, PERM_Edit_Home=:PERM_Edit_Home, PERM_View_Container=:PERM_View_Container, PERM_Edit_Container=:PERM_Edit_Container, PERM_Login=:PERM_Login, PERM_Manage_Staff=:PERM_Manage_Staff, PERM_Edit_Licence=:PERM_Edit_Licence, PERM_View_Logs=:PERM_View_Logs WHERE UID=:UID");
		$req->execute(array(
